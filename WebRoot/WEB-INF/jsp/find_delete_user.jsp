		...
		<table class="table table-border table-bordered table-bg table-hover table-sort">
			...
			<tbody>
				<tr class="text-c">
					...
					<td class="f-14 td-manage" id="delete" >
...
function article_edit(title,url){
	var sure=window.confirm("是否删除？");
	if(sure==true){
		var index = layer.open({
			type: 2,
			title: title,
			content: url+"?account="+$("#_account").html()
		});
		layer.full(index);
	}
}
	...
	findBtn.click(function(){
		$.post("basicInfoControl/find_update_user",{account:$("#vAccount").val()},function(data){
		 ...
		 if(vil.user==undefined){
			...
			$("#delete").html("<a style=\"text-decoration:none\" class=\"ml-5\" onClick=\"article_edit"+
								"('删除用户','basicInfoControl/delete_user')\" href=\"javascript:;\" title=\"删除用户\">"+
